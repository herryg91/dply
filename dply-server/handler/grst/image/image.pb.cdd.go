...
func RegisterImageApiGrstServer(grpcRestServer *grst.Server, hndl ImageApiServer) {
	...
	forward_ImageApi_Get_0 = grpcRestServer.GetForwardResponseMessage()

	forward_ImageApi_Add_0 = grpcRestServer.GetForwardResponseMessage()

	RegisterImageApiServer(grpcRestServer.GetGrpcServer(), hndl)
	grpcRestServer.RegisterRestHandler(RegisterImageApiHandler)
}
...
func NewImageApiGrstClient(serverHost string, creds *credentials.TransportCredentials, dialOpts ...grpc.DialOption) (ImageApiClient, error) {
	...
	return NewImageApiClient(grpcConn), err
}
